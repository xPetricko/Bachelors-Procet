...
class Agent():
    ...
    def update(self):
        ...
        s = T.FloatTensor([sars[0]
                           for sars in self.transition]).to(self.device)
        a = T.LongTensor([sars[1] for sars in self.transition]
                         ).view(-1, 1).to(self.device)
        r = T.FloatTensor([sars[3]
                           for sars in self.transition]).to(self.device)
        s_n = T.FloatTensor([sars[4]
                             for sars in self.transition]).to(self.device)
        d = T.FloatTensor([sars[2] for sars in self.transition]
                          ).view(-1, 1).to(self.device)

        d_r = [T.sum(T.FloatTensor([self.gamma**i for i in range(r[j:].size(0))])
                     * r[j:]) for j in range(r.size(0))]
        target_v = r.view(-1, 1) + \
            T.FloatTensor(d_r).view(-1, 1).to(self.device)
        ...
        policy_loss = - \
            probs.log_prob(a.view(a.size(0))).view(-1, 1) * advantage.detach()
